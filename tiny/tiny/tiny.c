...
int main(int argc, char **argv)
{
  ...
  if (argc != 2 && argc != 3)
  {
    fprintf(stderr, "usage: %s <port> (<thread>)\n", argv[0]);
    exit(1);
  }
  ...
  for (int i = 0; i < NUM_THREAD; i++)
    pthread_create(&th[i], NULL, serverthread, &v);
}
...
void *serverthread(void *args)
{
  while (1)
  {
    struct st *v = (struct st*) args;
    v->clen = sizeof(v->add);
    v->confd = Accept(v->listenfd, (SA *)&v->add, &v->clen); //line:netp:tiny:accept
    doit(v->confd);                                             //line:netp:tiny:doit
    Close(v->confd);                                            //line:netp:tiny:close
  }
}
/* $end tinymain */
